...
var GetList = function(that,tag = 0) {
  ...
  wx.request({
    url: base_url + 'home/WxRob/pagingOrder',
    data: {
      pageIndex: page,
      pageSize: pageSize,
      state: state,
...
Page({
  ...
  delete_order: function(event) {
    ...
    wx.showModal({
      ...
      success: function(res) {
        if (res.confirm) {
          wx.request({
            url: base_url + 'home/WxRob/deleteOrder',
            data: {
              id: oid
            },
    ...
    wx.showActionSheet({
      ...
      success (res) {
        if(res.tapIndex == 1){//余额支付
          tools.httpClient('home/WxRob/beforePay',{oid:oid},(error,data)=>{
            if(data.errorCode == 0){
              tools.httpClient('home/WxApp/payOrderByBalance',{oid:oid,type:2},(error,data)=>{
                if(data.errorCode == 0){
                  wx.showToast({
                    title: '支付成功',
                    duration: 3000,
                  ...
                  wx.request({
                    url: base_url + '../extend/pay/request/WxRobPay.php?oid=' + oid,
                    data: {
                      oid: oid
                    },
                    method: 'GET', // OPTIONS, GET, HEAD, POST, PUT, DELETE, TRACE, CONNECT
  ...
  write_off(event){
    var oid = event.currentTarget.dataset.id;
    wx.navigateTo({
      url: `/pages/writeOffCode/writeOffCode?oid=${oid}&type=2`
    });
  },
  ...
  orderDetail: function (e) {
    ...
    wx.navigateTo({
      url: '/pages/orderDetail/orderDetail?type=1&id=' + oid
    });
  }
})
...
function setOrderState(that, status, options) {
  wx.request({
    url: base_url + 'home/WxRob/setOrderState',
    data: {
      id: tem_id,
      state: status
    },
