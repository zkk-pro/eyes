...
var loadPage = function(that, options) {
	if (optionsData) {
		...
		wx.request({
			url: base_url + 'home/WxRob/getConfirmOrderData',
			data: myData,
			method: 'GET',
	...
	submitOrder: function(e) {
		...
		if(goodsList[0]['is_normal'] == 1){
			...
			subData.optometry_data = JSON.stringify({
				...
				"isDegree" : goodsList[0].is_data == 0 ? 3 : that.data.isDegree//用于判断是否需要填写验光数据
			});
		}else{
		...
		wx.request({
			url: base_url + 'home/WxRob/addOrder',
			data: subData,
			method: 'GET',
			success: function(res) {
				//console.log(res, 'klj');
									...
									success: function(res) {
										wx.requestPayment({
											...
											'success': function(res) {
												...
												// 			if (res.confirm) {
												// 				wx.redirectTo({
												// 					url: '/pages/rob/robOrder/robOrder?currentTab=a',
												// 				})
												// 			}
												// 		}
												// 	})
												// }else{
													wx.showModal({
														...
														success: function(res) {
															if (res.confirm) {
																wx.redirectTo({
																	url: '/pages/rob/robOrder/robOrder',
																})
															}
														}
													})
