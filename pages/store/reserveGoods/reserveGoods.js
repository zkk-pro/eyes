...
Page({
  ...
  onShow: function ( ) {
    ...
    wx.request({
      url: base_url + 'home/WxStore/reserveList',
      data: { account: userInfo.account },
      method: 'GET', // OPTIONS, GET, HEAD, POST, PUT, DELETE, TRACE, CONNECT
      success: function (res) {
        let dataList = res.data.data.dataList;
        let sum = 0; //购物车总价 不包含下架商品
        let total_num = 0;//购物车总数
        let con = '';
        if (dataList.length) {
          for (let i = 0; i < dataList.length; i++) { //计算总价 判断上下架
            ...
            if (dataList[i].property) {
              ...
              for (let m = 0; m < dataList[i].property.length; m++) {
  ...
  //删除按钮
  deleteReserveGoods: function (data) {
    let that = this;
    ...
    wx.showModal({
      ...
      success: function(res){
        ...
        if(res.confirm){
          wx.request({
            url: base_url + 'home/WxStore/deleteReserveGoods',
            data: {
              ids: ids,
  ...
  confirmOrder: function (data) {
    ...
    tools.isWxLogin(function(res) {
      if(res){
        ...
        wx.navigateTo({ url: '../reserveOrder/reserveOrder?gids=' + gids + '&gpids=' + gpids + '&counts=' + counts + '&cgids=' + cgids });
      }
    });//判断用户是否登录
  ...
  selectAll: function (e,data) {
    let that = this;
    let selectAll = that.data.selectAll;//获取当前的全选状态
    ...
    let dataList = that.data.dataList;//获取购物车列表
  ...
  check: function (e,data) {
    let that = this;
    let index = e.currentTarget.dataset.index;//获取当前index
    let dataList = that.data.dataList;//获取购物车列表
    let selected = dataList[index].checked;  //获取当前选中状态
    // 改变状态
    if (selected == true){
  ...
  add: function (e,data) {
    ...
    let gpid = e.currentTarget.dataset.gpid;
    let account = userInfo.account;
    let count = parseInt(e.currentTarget.dataset.count) + 1;
    tools.httpClient('home/WxStore/addReserveGoods',{
      gid: gid,
      account: account,
      goods_count: 1,
      gpid: gpid
...
//编辑 完成 切换
  changeTab:function(e){
    let ee = e.currentTarget.dataset.editend;
    if(ee == 0){
      this.setData({
        editEnd:1,
        editEndText:'编辑',
