...
var GetList = function(that,tag = 0) {
  ...
  wx.request({
    url: base_url + 'home/WxStore/pagingOrder',
    data: {
      pageIndex: page,
      pageSize: pageSize,
      state: state,
    ...
    success: function(res) {
      ...
      optometristInfo =  wx.getStorageSync('optometristInfo');
      if(tag == 1){
        var	list = [];
      }else{
      ...
      for (var i = 0; i < dataList.length; i++) {
        ...
        switch (order_state) { //得到不同订单状对应的 操作和 状态文字
          case '0':
            ...
            break;
          case '1':
            if(!dataList[i].optometry_data || optometry_data.isDegree != 3){
              dataList[i]['btn_hiddens'] = true;
              dataList[i]['state_text'] = "待发货";
              dataList[i]['option'] = 'refund_order';
              dataList[i]['option_text'] = '申请退款';
              if(dataList[i].optometry_data && dataList[i].is_time_over == 1){
                dataList[i]['option'] = 'showTips';
                dataList[i]['option_text'] = '产品制作中';
              }
            }else if (!dataList[i].is_buy || !optometristInfo) {
              dataList[i]['btn_hidden'] = dataList[i]['no_refund'] == 1 ? false : true;
              dataList[i]['btn_hiddens'] = true;
              dataList[i]['state_text'] = dataList[i]['no_refund'] == 1 ? '待核销(退款失败)': "待核销";
              dataList[i]['option'] = 'write_off';
              dataList[i]['option_text'] = '核销';
              dataList[i]['options'] = 'refund_order';
              dataList[i]['option_texts'] = '申请退款';
              if(dataList[i].optometry_data && dataList[i].is_time_over == 1){
                dataList[i]['options'] = 'showTips';
                dataList[i]['option_texts'] = '产品制作中';
              }
            }
...
/**
  ...
  delete_order: function(event) {
    ...
    wx.showModal({
      ...
      success: function(res) {
        if (res.confirm) {
          wx.request({
            url: base_url + 'home/WxStore/deleteOrder',
            data: {
              id: oid
            },
    ...
    wx.showActionSheet({
      ...
      success (res) {
        if(res.tapIndex == 1){//余额支付
          tools.httpClient('home/WxStore/beforePay',{oid:oid},(error,data)=>{
            if(data.errorCode == 0){
                tools.httpClient('home/WxApp/payOrderByBalance',{oid:oid,type:1},(error,data)=>{
                  if(data.errorCode == 0){
                    wx.showToast({
                      title: '支付成功',
                      duration: 3000,
  ...
  write_off(event){
    var oid = event.currentTarget.dataset.id;
    wx.navigateTo({
      url: `/pages/writeOffCode/writeOffCode?oid=${oid}&type=1`
    });
  },
  ...
  orderDetail: function (e) {
    ...
    wx.navigateTo({
      url: '/pages/orderDetail/orderDetail?type=0&id=' + oid
    });
  }
})
...
function setOrderState(that, status, options) {
  wx.request({
    url: base_url + 'home/WxStore/setOrderState',
    data: {
      id: tem_id,
      state: status
    },
