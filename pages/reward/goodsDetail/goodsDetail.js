...
Page({
    ...
    onLoad: function (options) {
        ...
        //获取商品详情
        wx.request({
            url: base_url + 'home/WxReward/getGoodsDetail',
            ...
            success: function (res) {
                ...
                that.setData({
                    ...
                    rate: Number((1 / parseInt(data.inventory) * 100).toFixed(2)) + '%',
    ...
    onShow: function () {
        ...
        //获取商品属性
        wx.request({
            url: base_url + 'home/WxReward/getPropertyGroup',
            data: {gid: gid},
            method: 'GET',
            success: function (res) {
                var data = res.data.data;
    ...
    share() {
        tools.isWxLogin(function (res) {
            wx.navigateTo({
                url: '/pages/goodShare/goodShare?type=2&gid=' + gid,
            })
        })
    ...
    goodsCount: function (e) {
        ...
        let inventory = parseInt(that.data.inventory);

        if (optType == 1) {
            goodsCount++;
            let rate = Number((goodsCount / inventory * 100).toFixed(2)) + '%';
            var price = (goodsCount * Number(that.data.singlePrice)).toFixed(2);
            that.setData({count: goodsCount, propertyPrice: price, rate: rate});
        } else {
            goodsCount--;
            if (goodsCount < 1) {
                goodsCount = 1;
    ...
    onShareAppMessage: function (res) {
        ...
        return {
            ...
            path: '/pages/reward/goodsDetail/goodsDetail?gid=' + gid + '&user_id=' + userInfo.id,
            success: function (res) {
                // 转发成功
            },
            fail: function (res) {
