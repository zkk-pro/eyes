...
Page({
   ...
   */
  getShareCode(that){
    tools.httpClient('home/WxApp/getCode',{user_id:userInfo.id,type:11},(error,data)=>{
      if(data.errorCode == 0){
  ...
  beforeDrawImg(that){
    wx.getImageInfo({
      src: base_url + 'bg/in-bg.png?r=' + Math.random(),
      success(e){
        bg = e.path;
        that.drawImg();
      }
  ...
  drawImg() {
    ...
    context.drawImage(codeImg, 200, 580,350, 350) //绘制二维码
