...
Page({
   ...
   */
  getShareCode(that){
    tools.httpClient('home/WxApp/getCode',{user_id:userInfo.id,type:3},(error,data)=>{
      if(data.errorCode == 0){
  ...
  beforeDrawImg(that){
    wx.getImageInfo({
      src: base_url + 'bg/bgNew.png?r=' + Math.random(),
      success(e){
        bg = e.path;
        that.drawImg();
      }
  ...
  drawImg() {
    ...
    context.drawImage(codeImg, 460, 750,260, 245) //绘制二维码
